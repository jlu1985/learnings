...
public class ClassFileMagicNumberChecker {
	...
	// note that this is an elementary version of a checker that checks if the
	// given file
	...
	public static void main(String[] args) {
		if (args.length != 1) {
			System.err.println("Pass a valid file naem as argument");
			System.exit(-1);
		}
		String fileName = args[0];
		// create a magicNumber byte array with values for four bytes in
		// 0xCAFEBABE
		...
		try (FileInputStream fis = new FileInputStream(fileName)) {
			...
			if (fis.read(u4buffer) != -1) { // if read was successful
				...
				// checks for euqality of contents
				if (Arrays.equals(magicNumber, u4buffer)) {
					System.out
							.printf("The magic number for passed file %s matches that of a .class file",
									fileName);
				} else {
					System.out
							.printf("The magic number for passed file %s does not match that of a .class file",
									fileName);
				}
			}
			...
			System.err
					.println("an I/O error occurred whilte processing the file");
		}
	}
